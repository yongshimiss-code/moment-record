...
const state = {
  ...
  textAiSaveStatus: "",
  voiceDraft: "",
  voiceAiMode: "summarize",
  voiceAiResult: "",
  voiceAiGenerated: false,
  voiceAiAccepted: false,
  voiceAiOriginal: "",
  voiceSavePickerOpen: false,
  voiceAiWorking: false,
  voiceAiAccepting: false,
  voiceAiSaveStatus: "",
  expandedNoteIds: [],
...
const voiceInlineActions = document.getElementById("voice-inline-actions");
const voiceAiResultWrap = document.getElementById("voice-ai-result-wrap");
const voiceAiResultEl = document.getElementById("voice-ai-result");
const voiceAiFooter = document.getElementById("voice-ai-footer");
const voiceAiSaveField = document.getElementById("voice-ai-save-field");
const voiceAiBookSelect = document.getElementById("voice-ai-book-select");
const voiceAiSaveWrap = document.getElementById("voice-ai-save-wrap");
const voiceAiSaveStatusEl = document.getElementById("voice-ai-save-status");
const voiceAiCancelButton = document.getElementById("voice-ai-cancel-button");
const voiceAiRegenerateButton = document.getElementById("voice-ai-regenerate-button");
const voiceAiAcceptButton = document.getElementById("voice-ai-accept-button");
const voiceAiSaveButton = document.getElementById("voice-ai-save-button");
const voiceAiNewBookButton = document.getElementById("voice-ai-new-book-button");
const voiceDirectSaveButton = document.getElementById("voice-direct-save-button");
...
function getCurrentVoiceCaptureValue() {
  return voiceTextarea?.value.trim() || state.voiceDraft.trim();
}
...
function updateVoiceUI() {
  ...
  if (voiceProcessButton) {
    voiceProcessButton.hidden = true;
...
function populateVoiceAiBookSelect() {
  if (!voiceAiBookSelect) {
    return;
  }
  ...
  state.processingSaveBookId = selectedBookId;
  voiceAiBookSelect.innerHTML = visibleBooks.map((book) => `
    <option value="${book.id}" ${book.id === selectedBookId ? "selected" : ""}>${book.title}</option>
  `).join("");
}

function resetVoiceAiInline() {
  state.voiceAiOriginal = "";
  state.voiceAiResult = "";
  state.voiceAiGenerated = false;
  state.voiceAiAccepted = false;
  state.voiceSavePickerOpen = false;
  state.voiceAiWorking = false;
  state.voiceAiAccepting = false;
  state.voiceAiSaveStatus = "";
  if (voiceAiBookSelect) {
    voiceAiBookSelect.selectedIndex = Math.max(voiceAiBookSelect.selectedIndex, 0);
  }
  renderVoiceAiInline();
}

function renderVoiceAiInline() {
  const hasGeneratedResult = state.voiceAiGenerated && Boolean(state.voiceAiResult.trim());
  const hasDraftText = Boolean((voiceTextarea?.value || state.voiceDraft || "").trim());
  const hasTranscript = Boolean(state.voiceTranscript.trim()) || hasDraftText || Boolean(state.voiceAudioUrl);

  if (voiceTextField) {
    voiceTextField.hidden = !hasTranscript;
  }
  if (voiceInlineActions) {
    voiceInlineActions.hidden = !hasTranscript;
  }
  if (voiceTextarea) {
    voiceTextarea.hidden = hasGeneratedResult;
  }
  if (voiceAiResultWrap) {
    voiceAiResultWrap.hidden = !hasGeneratedResult;
    voiceAiResultWrap.classList.toggle("is-accepting", state.voiceAiAccepting);
  }
  if (voiceAiResultEl) {
    voiceAiResultEl.innerHTML = hasGeneratedResult
      ? buildInlineTextAiPreview(state.voiceAiOriginal, state.voiceAiResult)
      : "";
  }
  if (voiceAiFooter) {
    voiceAiFooter.hidden = !hasGeneratedResult;
  }
  if (voiceAiSaveField) {
    voiceAiSaveField.hidden = !state.voiceSavePickerOpen;
  }
  if (voiceAiSaveWrap) {
    voiceAiSaveWrap.hidden = !state.voiceSavePickerOpen;
  }
  document.querySelectorAll("[data-voice-ai-mode]").forEach((button) => {
    button.disabled = state.voiceAiWorking || !hasDraftText;
    button.classList.toggle("is-active", button.dataset.voiceAiMode === state.voiceAiMode);
  });
  if (voiceAiCancelButton) {
    voiceAiCancelButton.disabled = state.voiceAiWorking || !hasGeneratedResult;
  }
  if (voiceAiRegenerateButton) {
    voiceAiRegenerateButton.disabled = state.voiceAiWorking || !hasDraftText;
    voiceAiRegenerateButton.textContent = state.voiceAiWorking
      ? localizeText("Generating...", "生成中...")
      : localizeText("Regenerate", "重新生成");
  }
  if (voiceAiAcceptButton) {
    voiceAiAcceptButton.disabled = state.voiceAiWorking || !hasGeneratedResult;
  }
  if (voiceDirectSaveButton) {
    voiceDirectSaveButton.disabled = !hasDraftText;
  }
  if (voiceAiSaveButton) {
    voiceAiSaveButton.disabled = !state.voiceSavePickerOpen || !hasDraftText;
  }
  if (voiceAiNewBookButton) {
    voiceAiNewBookButton.disabled = !state.voiceSavePickerOpen || !hasDraftText;
  }
  if (voiceAiSaveStatusEl) {
    voiceAiSaveStatusEl.textContent = state.voiceAiSaveStatus;
  }
}

function syncVoiceDraftFromTranscript() {
  const nextText = state.voiceTranscript.trim();
  if (!nextText) {
    return;
  }
  state.voiceDraft = nextText;
  if (voiceTextarea && document.activeElement !== voiceTextarea) {
    voiceTextarea.value = nextText;
  }
  state.voiceAiOriginal = "";
  state.voiceAiResult = "";
  state.voiceAiGenerated = false;
  ...
  state.voiceSavePickerOpen = false;
  state.voiceAiSaveStatus = "";
  populateVoiceAiBookSelect();
  renderVoiceAiInline();
}

function cancelInlineVoiceAi() {
  state.voiceAiResult = "";
  state.voiceAiGenerated = false;
  state.voiceAiAccepted = false;
  state.voiceAiAccepting = false;
  ...
  state.voiceAiSaveStatus = "";
  if (voiceTextarea) {
    voiceTextarea.value = state.voiceAiOriginal || state.voiceDraft || voiceTextarea.value;
  }
  state.voiceDraft = state.voiceAiOriginal || state.voiceDraft;
  renderVoiceAiInline();
}

async function generateInlineVoiceAi(mode = state.voiceAiMode) {
  const text = voiceTextarea?.value.trim()
    || state.voiceDraft
    || state.voiceTranscript.trim()
    || localizeText("Record a thought here, then refine it with AI.", "先录下一点想法，再用 AI 帮你整理。");

  state.voiceDraft = text;
  state.voiceAiOriginal = text;
  state.voiceAiMode = mode;
  state.voiceAiResult = "";
  state.voiceAiGenerated = false;
  state.voiceAiAccepted = false;
  state.voiceAiAccepting = false;
  state.voiceSavePickerOpen = false;
  state.voiceAiSaveStatus = "";
  state.voiceAiWorking = true;
  renderVoiceAiInline();

  const fallback = mode === "expand"
    ? localizeText(
      `${text}\n\nA useful next step is to connect this spoken thought to one precise example from the book, so the note keeps its warmth while becoming clearer.`,
      `${text}\n\n一个自然的下一步，是把这段口述想法连到书里的一个具体例子上，让它既保留口语的温度，也变得更清晰。`
    )
    : localizeText(
      clipText(text, 180),
      clipText(text, 120)
  ...
  try {
    ...
    if (hasLiveAiConfig()) {
      const modePrompt = mode === "expand"
        ? "Expand the spoken reading note into a slightly richer, clearer version while keeping it concise. Return only the rewritten note."
        : "Summarize the spoken reading note into a concise, clean version. Return only the rewritten note.";
      const reply = await callAi([
        {
          role: "system",
          content: modePrompt
    ...
    state.voiceAiResult = result;
    state.voiceAiGenerated = Boolean(result.trim());
  } catch (error) {
    state.voiceAiResult = fallback;
    state.voiceAiGenerated = Boolean(fallback.trim());
  } finally {
    state.voiceAiWorking = false;
    renderVoiceAiInline();
  }
...
function acceptInlineVoiceAi() {
  if (!state.voiceAiResult.trim()) {
    return;
  }

  const confirmedText = state.voiceAiResult.trim();
  state.voiceAiAccepted = true;
  state.voiceAiAccepting = true;
  renderVoiceAiInline();

  window.setTimeout(() => {
    if (voiceTextarea) {
      voiceTextarea.value = confirmedText;
    }
    state.voiceDraft = confirmedText;
    state.voiceAiOriginal = "";
    state.voiceAiResult = "";
    state.voiceAiGenerated = false;
    state.voiceAiAccepted = false;
    state.voiceAiAccepting = false;
    state.voiceAiSaveStatus = localizeText("AI 修改已覆盖语音转写。", "AI 修改内容已覆盖语音转写。");
    renderVoiceAiInline();
  }, 180);
}

function openVoiceSavePicker() {
  const text = getCurrentVoiceCaptureValue();
  if (!text) {
    return;
  }
  ...
  state.voiceSavePickerOpen = true;
  state.voiceAiSaveStatus = localizeText(
    "Choose whether to save into an existing book or create a new one.",
    "选择保存到现有书本，或者新建一本书。"
  );
  if (state.captureEntryBookId) {
...
function saveInlineVoiceToBook() {
  const selectedBookId = voiceAiBookSelect?.value || state.processingSaveBookId || state.activeBookId;
  const text = getCurrentVoiceCaptureValue();
  if (!selectedBookId || !text) {
    return;
  }

  saveCaptureToBook(selectedBookId, "voice", text, {
    statusMessage: localizeText(
      `Saved to ${books.find((book) => book.id === selectedBookId)?.title || ""}.`,
      `已保存到《${localizeBookTitle(books.find((book) => book.id === selectedBookId)?.title || "")}》。`
    )
  });
  state.voiceAiSaveStatus = localizeText("Saved to your book.", "已保存到书本。");
  state.voiceAiAccepted = false;
  state.voiceAiAccepting = false;
  state.voiceAiGenerated = false;
  state.voiceAiOriginal = "";
...
function processVoiceCapture() {
  ...
  state.voiceDraft = source;
  if (voiceTextarea) {
    voiceTextarea.value = source;
  }
  state.pendingBookNote = {
    source: "voice",
    text: source
...
function applyLanguage() {
  ...
  setText("[data-voice-ai-mode='summarize'] .ai-mini-btn__label", t("suggestion_summarize"));
  setText("[data-voice-ai-mode='expand'] .ai-mini-btn__label", t("suggestion_expand"));
  setText("#voice-direct-save-button .ai-mini-btn__label", localizeText("Save", "保存"));
  setText("#voice-ai-cancel-button", localizeText("Cancel", "取消"));
  setText("#voice-ai-regenerate-button", localizeText("Regenerate", "重新生成"));
  setText("#voice-ai-accept-button", localizeText("Confirm", "确认"));
  setText("#voice-ai-new-book-button", t("new_book"));
  setText("#voice-ai-save-button", localizeText("Save to existing book", "保存到现有书本"));
  setText("#voice-ai-save-field span", t("save_into_book"));
...
document.addEventListener("click", async (event) => {
  ...
  const voiceAiModeButton = event.target.closest("[data-voice-ai-mode]");
  if (voiceAiModeButton) {
    await generateInlineVoiceAi(voiceAiModeButton.dataset.voiceAiMode || "summarize");
    return;
  }

  const voiceAiRegenerate = event.target.closest("#voice-ai-regenerate-button");
  if (voiceAiRegenerate) {
    await generateInlineVoiceAi(state.voiceAiMode || "summarize");
    return;
  }

  const voiceAiCancel = event.target.closest("#voice-ai-cancel-button");
  if (voiceAiCancel) {
    cancelInlineVoiceAi();
    return;
  }

  const voiceAiAccept = event.target.closest("#voice-ai-accept-button");
  if (voiceAiAccept) {
    acceptInlineVoiceAi();
    return;
  }

  const voiceDirectSave = event.target.closest("#voice-direct-save-button");
  if (voiceDirectSave) {
    openVoiceSavePicker();
    return;
  }

  const voiceAiSave = event.target.closest("#voice-ai-save-button");
  if (voiceAiSave) {
    saveInlineVoiceToBook();
    return;
  }

  const voiceAiNewBook = event.target.closest("#voice-ai-new-book-button");
  if (voiceAiNewBook) {
    const text = getCurrentVoiceCaptureValue();
    state.pendingNewBookSource = "voice";
    state.pendingBookNote = {
      source: "voice",
      text
    };
...
if (voiceTextarea) {
  voiceTextarea.addEventListener("input", () => {
    state.voiceDraft = voiceTextarea.value.trim();
    state.voiceAiOriginal = "";
    state.voiceAiResult = "";
    state.voiceAiGenerated = false;
    state.voiceAiAccepted = false;
...
if (voiceAiBookSelect) {
  voiceAiBookSelect.addEventListener("change", () => {
    state.processingSaveBookId = voiceAiBookSelect.value;
    state.voiceAiSaveStatus = "";
    renderVoiceAiInline();
  });
}
